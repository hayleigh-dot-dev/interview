    ...
    let
        ...
        statementInfo =
            Html.div [ Html.Attributes.class "flex h-96 overflow-y-scroll mb-4" ]
                (selected
                    |> Maybe.map viewSplitStatementInfo
                    |> Maybe.withDefault [ Html.text "" ]
                )
    ...
    Json.Decode.field "type" Json.Decode.string
        |> Json.Decode.andThen
            (\t ->
                case t of
                    "BasicSort" ->
                        Json.Decode.map BasicSort <|
                            Json.Decode.map4 basicData
                    ...
                    "NormalSort" ->
                        Json.Decode.map NormalSort <|
                            Json.Decode.map5 normalData
                                (Json.Decode.field "title" Json.Decode.string)
                                (Json.Decode.field "description" Json.Decode.string)
                                (Json.Decode.field "statements" <|
                                    Json.Decode.list <|
        ...
        |> Json.Decode.andThen
            (\r ->
                case r of
                    ...
                    _ ->
                        Json.Decode.fail <| "Unknown rating: " ++ r
            )
